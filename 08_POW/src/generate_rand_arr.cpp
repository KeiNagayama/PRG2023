...
// ===== Ex.4 =====
// Pow(alpha,xmin) by inverse function method
// CDF   : F(x) := P(>x) = e^{-x/mu}    ; For x > 0
// InvCDF: x = F^{-1}(u) = - mu * log(u); For u in (0,1]
std::vector<double> generate_pow_rand_arr(std::mt19937 &mt, int N, double alpha = 1, double xmin = 1)
{
	std::uniform_real_distribution<double> uni(0.0, 1.0);
	std::vector<double> rand_arr;
	auto inv_pow = [&alpha, &xmin](double u)
	{ return xmin * std::pow(1 - u, -1 / alpha); };
	for (int i = 0; i < N; i++)
	{
		// uni(mt)   \in [0,1)
		// 1-uni(mt) \in (0,1]
		rand_arr.push_back(inv_pow(1 - uni(mt)));
	}
	return rand_arr;
}
...
int main(int argc, char *argv[])
{
	if (argc < 3)
	{
		...
		std::cout << "Usage: <bin> <out_dir> <seed>" << std::endl;
		return 0;
	}
	...
	std::string out_pow = out_dir + "pow";
	...
	double alpha = 1;
	double xmin = 1;
	rand_arr = generate_pow_rand_arr(mt, N, alpha, xmin);
	output_vector(rand_arr, out_pow);
